...
investor_df_og = new_portfolio.sheets['Ledger'].range('H4:O8').options(pd.DataFrame, header=1, index=False).value
investor_df = investor_df_og[['Investor', 'Amount Invested', 'Profit/Loss']]
...
risk_table_df = new_portfolio.sheets['Ledger'].range('P6:U8').options(pd.DataFrame, header=1, index=False).value
...
app.layout = html.Div(style={'fontFamily': 'Lato, Roboto, sans-serif', 'backgroundColor': colors['background'], 'margin':'1%'},
children = [
...
@app.callback(Output('returns', 'children'),
            Input('interval-component', 'n_intervals' ))

#Displays the return percentage of the portfolio
def returns_display(n):
    wb = new_portfolio
    total_returns = wb.sheets['Portfolio']['A13'].value
    total_returns = f"{total_returns:.2%}"
    return html.H3(f"{total_returns}")

@app.callback(Output('returns', 'style'),
    Input('interval-component', 'n_intervals'))

#Changes the colour of returns percentage
def update_div_style(n):
    wb = new_portfolio
    total_returns = 100 * wb.sheets['Portfolio']['A13'].value
    if total_returns < 0:
        return {'color': 'red'}
    else:
        return {'color': 'green'}

@app.callback(Output('live-update-price_portfolio', 'style'),
    Input('interval-component', 'n_intervals'))

#Changes the colour of the portfolio value
def update_div_style(n):
    wb = new_portfolio
    total_returns = 100 * wb.sheets['Portfolio']['A13'].value
    if total_returns < 0:
...
@app.callback(Output('live-update-price-nifty', 'children'),
              Input('interval-component', 'n_intervals'))
...
@app.callback(Output('live-update-price_portfolio', 'children'),
              Input('interval-component', 'n_intervals'))

def update_live_price(n):
    wb = new_portfolio
    current_price, investment_value, total_profit_loss = [[] for i in range(3)]
    portfolio_sum = wb.sheets['Funds_Portfolio']['A7'].value
    total_profit = wb.sheets['Funds_Portfolio']['A10'].value
    i = 0
    ...
    wb.sheets['Portfolio']['A14'].value = total_profit
    wb.sheets['Portfolio']['A4'].value = portfolio_sum
    current_price= f"{portfolio_sum:,.2f}"
    
    return html.H3(f"₹ {current_price}")


@app.callback(Output('live-update-nav','children'),
            Input('interval-component', 'n_intervals'))

#Displays the NAV
def display_nav(n):
    wb = new_portfolio
    nav = wb.sheets['Portfolio']['A25'].value
    nav = f"{nav:.2f}"
...
#Function to plot the NIFTY 50 and NAV of the portfolio
def nifty_and_nav(n, value):
    ...
    last_row = wb.sheets['Log'].range('D1048576').end('up').row
    log_df = wb.sheets['Log'].range('C4:I'+str(last_row)).options(pd.DataFrame, header=1, index=False).value
...
@app.callback(Output('graph-sector-returns','figure'),
                    Input('dropdown-selection-sector-returns', 'value'))

#Displays the returns by sector
