...
#stock_tickers = "TATAPOWER.NS, POWERGRID.NS, RELIANCE.NS, TMPV.NS, IOC.NS, LEMONTREE.NS, ETERNAL.NS, PPLPHARMA.NS, GOLDBEES.NS, BFSI.NS, NIFTYBEES.NS, MOSMALL250.NS, HDFCMID150.NS, INDUSTOWER.NS"
...
def add_data_to_portfolio(stock_tickers, money_invested_sum):
    ...
    current_price, change, two_hundred_day_average, market_cap, fifty_two_week_high, fifty_two_week_low, investment_value, today_profit_loss, today_profit_loss_perc, total_profit_loss, total_profit_loss_perc = [
        [] for i in range(11)]

    portfolio_sum = new_portfolio.sheets['Funds_Portfolio']['A7'].value
    ...
    #Total Stocks:
    total_stocks = new_portfolio.sheets['Funds_Portfolio']['A13'].value
    ...
    #Returns
    total_profit = new_portfolio.sheets['Funds_Portfolio']['A10'].value
    ...
    sheet1.range('K3:K' + str(last_row_portfolio)).options(transpose=True).formula = '=J3/A$7'
...
def create_book():
    ...
    #new_portfolio = xw.Book('Book1.xlsx')
...
def excel_reader():
    ...
    money_invested_sum = new_portfolio.sheets['Portfolio']['A26'].value
...
def initial_stock_tickers():
    ...
    print(
        "Enter the intial ticker symbols in one sentence below, seperating each symbol with a comma, like so: AAPL, GOOGL ")
...
def risk_table():
    ...
    sheet_ledger['U7'].formula = '=V7/S4'
    sheet_ledger['U8'].formula = '=V8/S4'
...
def update_beta_sheet(stock_names):
    ...
    last_update = new_portfolio.sheets['Beta'][
        'J8'].value  #new_portfolio.sheets['Beta']['J'+str(last_row_beta_sheet)].value
    ...
    prices = list(stocks_history_download['^NSEI'][0:])
    ...
    for name in stock_names:
        ...
        new_portfolio.sheets['Beta'].range((8, i)).options(transpose=True).value = list(temp_df['Change %'])
        i += 1

    new_portfolio.sheets['Beta']['J6'].value = 'Beta'
    new_portfolio.sheets['Beta']['J7'].value = 'Date'
    new_portfolio.sheets['Beta']['J8'].options(transpose=True).value = list(temp_df['Date'])
...
def update_ledger():
    ...
    last_row = new_portfolio.sheets['Ledger'].range('A1048576').end('up').row
    print("If an investor has paid, add the amount that they have paid.\nIf they haven't paid, input the amount as 0\n")
    time_period = input("\nEnter the period in the following format Aug-24: ")
    i = 0
    for investor in ledger_df['Investor']:
        money_added = float(input(f'\nEnter the amount {investor} has invested in {time_period}: '))
        if money_added > 0:
...
def update_log():
    ...
    prev_portfolio_value = new_portfolio.sheets['Log']['D' + str(last_row)].value
    date_and_time = datetime.datetime.now()
    ...
    new_portfolio.sheets['Log']['E' + str(new_row)].value = yf.Ticker('^NSEI').get_info()['regularMarketPrice']
    new_portfolio.sheets['Log']['F' + str(new_row)].value = new_portfolio.sheets['Portfolio']['A25'].value
    new_portfolio.sheets['Log']['G' + str(new_row)].value = portfolio_value / prev_portfolio_value - 1
    new_portfolio.sheets['Log']['H' + str(new_row)].value = new_portfolio.sheets['Log']['E' + str(new_row)].value / \
                                                            new_portfolio.sheets['Log']['E' + str(last_row)].value - 1
    new_portfolio.sheets['Log']['I' + str(new_row)].value = new_portfolio.sheets['Log']['F' + str(new_row)].value / \
                                                            new_portfolio.sheets['Log']['F' + str(last_row)].value - 1
...
def update_sheets(stock_names):
    ...
    for name in stock_names:
        ...
        if datetime.datetime.now().strftime("%Y-%m-%d") != last_update:
            ...
            new_portfolio.sheets[name]['H' + str(last_row + 1)].value = dates
            new_portfolio.sheets[name]['I' + str(last_row + 1)].value = prices
...
def update_transactions():  #Function to add new entries to new_portfolio[Transaction History]

    money_invested_sum = new_portfolio.sheets['Portfolio']['A7'].value
...
new_portfolio = xw.books['Book1_Archive.xlsx']
